...
**行内元素设置上下的padding是无效果的,可以设置左右padding**
...
~~~css
box-shadow:水平阴影 垂直阴影 模糊距离（虚实,值越大,越模糊）  阴影尺寸（影子大小）  阴影颜色  内/外阴影；
~~~
...
浮动的盒子不会继承父盒子的宽高,如果没有宽高会由内容撑开

浮：    加了浮动的元素盒子是浮起来的，漂浮在其他的标准流盒子上面。
漏：    加了浮动的盒子，不占位置的，它浮起来了，它原来的位置漏 给了标准流的盒子。
特：    特别注意，首先浮动的盒子需要和标准流的父级搭配使用， 其次 特别的注意浮动可以使元素显示模式体现为行内块特性。

当父盒子的宽度不足以放下浮动的子元素时,会掉下来,但掉下来的元素,会跟前一个元素的底边线对齐.
...
缺点： 在有依据父元素定位的元素时,在父盒子外面的时候overflow:hidden会把定位的元素隐藏掉
...
在页面中可以节省标签,页面中的一些有图标,图片可以使用伪元素代替

```css
 选择器 ：：before  ===>>  在当前元素的子元素前面添加一个标签

 选择器 ：：after  ===>>  在当前元素的子元素后面添加一个标签

 content属性不能省略

 相当于一个正常的标签，可以设置CSS样式
```
...
<img src="D:/%E4%B8%8A%E6%B5%B7%E9%BB%91%E9%A9%AC%E5%AD%A6%E4%B9%A0/%E5%9F%BA%E7%A1%80%E7%8F%AD%E7%AC%94%E8%AE%B0/media/yl.jpg" width="400" />

最普通的，最为常用的结构

## 两列左窄右宽型

<img src="D:/%E4%B8%8A%E6%B5%B7%E9%BB%91%E9%A9%AC%E5%AD%A6%E4%B9%A0/%E5%9F%BA%E7%A1%80%E7%8F%AD%E7%AC%94%E8%AE%B0/media/ll.jpg" width="400" />

比如小米    <a href="http://www.mi.com" target="_blank"> 小米官网 </a>

## 通栏平均分布型

<img src="D:/%E4%B8%8A%E6%B5%B7%E9%BB%91%E9%A9%AC%E5%AD%A6%E4%B9%A0/%E5%9F%BA%E7%A1%80%E7%8F%AD%E7%AC%94%E8%AE%B0/media/tl.jpg" width="600" />

比如锤子    <a href="http://www.smartisan.com/" target="_blank"> 锤子官网 </a>

## 版心的特殊情况

当网页中有一张特别大的图片,版心放不下的时候,就会产生横向滚动条

当背景是一张超大的图片的时候,使用background,但背景图片需要宽高,宽可以继承,但是一定要给盒子高度

高度和图片一样高

```css
background: url(images/...)  center 0 no-repeat;

/*大图一般自带版心,center可以让背景图水平居中*/
```
...
ctrl + r  显示隐藏标尺        ctrl + k 标尺 --- 把里面的单位一律改为像素
...
C + 鼠标左键选中===  > 切片工具
 ...
 ctrl +shift + alt + S====> 保存切片

I 工具====>吸管工具

ctrl  按住不放  ====>在切片状态下可变成切片选择工具
...
5. 当存在嵌套关系使用定位,当父元素之间已经比较过(Z-index),两个父元素都写了z-index,子元素与子元素之间不会在比较层级关系,层级关系由父级的层级关系决定
...
2. 搜索自己想要的图标  添加入库
3. 点击下载代码 (需要登录的)
4. 区分对应文件 (文件夹中不识别的后缀名都是字体文件)
5. 打开demo_index.html
6. 选择font_class方式应用字体图标
7. 引用对应的iconfont.css文件 (注意路径)
8. 挑选对应的图标并获取类名 (iconfont类名不能省略 固定的)
```
...
```
  项目文件夹   ：最大的文件夹，所有的项目文件都存放在这个文件夹里面 例如：JD
...
reset.css/(base.css)       初始化css,将不同全部干掉
common.css   	公共样式文件(网页的头部和底部样式)
